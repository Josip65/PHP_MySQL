<?php 
	...
	if (isset($_POST['_action_']) && $_POST['_action_'] == 'add_news') {
        ...
        if($_FILES['picture']['error'] == UPLOAD_ERR_OK && $_FILES['picture']['name'] != "") {
            ...
            $_picture = "images/". $ID . '-' . rand(1,100) . $ext;
	...
	if (isset($_POST['_action_']) && $_POST['_action_'] == 'edit_news') {
        ...
        if($_FILES['picture']['error'] == UPLOAD_ERR_OK && $_FILES['picture']['name'] != "") {
			...
			$_picture = "images/". (int)$_POST['edit'] . '-' . rand(1,100) . $ext;
			copy($_FILES['picture']['tmp_name'],  $_picture);
		...
		$_SESSION['message'] = '<p>Uspješno ste uredili novost!</p>';
	...
	#Add news 
		...
		<form action="" id="news_form" name="news_form" method="POST" enctype="multipart/form-data">
			...
			<input type="text" id="title" name="title" placeholder="News title.." required><br/><br/>
			<label for="description">Opis *</label><br/>
			<textarea id="description" name="description" placeholder="News description.." required></textarea><br/><br/>
	...
	#Edit news
		...
		<form action="" id="news_form_edit" name="news_form_edit" method="POST" enctype="multipart/form-data">
			...
			<label for="title">Naslov *</label><br/>
			<input type="text" id="title" name="title" value="' . $row['title'] . '" placeholder="News title.." required><br/><br/>
			<label for="description">Opis *</label><br/>
			<textarea id="description" name="description" placeholder="News description.." required>' . $row['description'] . '</textarea><br/><br/>
		...
		<div id="news">
			...
			<a href="index.php?menu=' . $menu . '&amp;action=' . $action . '&amp;add=true" class="AddLink">Dodajte novost</a>
		</div>';
	}
