<?php 
	...
	if (isset($_POST['edit']) && $_POST['_action_'] == 'TRUE') {
		$query  = "UPDATE users SET ime='" . $_POST['ime'] . "', prezime='" . $_POST['prezime'] . "', email='" . $_POST['email'] . "', username='" . $_POST['username'] . "', country='" . $_POST['country'] . "', archive='" . $_POST['archive'] . "'";
		...
		$_SESSION['message'] = '<p>Uspješno ste promijenili podatke na profilu!</p>';
	...
	if (isset($_GET['delete']) && $_GET['delete'] != '') {
		...
		$query .= " LIMIT 1";
		$result = @mysqli_query($MySQL, $query);
		$_SESSION['message'] = '<p>Uspješno ste obrisali korisnički račun!</p>';
	...
	#Edit user profile
		...
		print '
		<h2>Promjena podataka korisničkog računa</h2>
		<form action="" id="registration_form" name="registration_form" method="POST">
			<input type="hidden" id="_action_" name="_action_" value="TRUE">
			<input type="hidden" id="edit" name="edit" value="' . $_GET['edit'] . '">
			
			<label for="fname">Ime *</label><br/>
			<input type="text" id="fname" name="ime" value="' . $row['ime'] . '" placeholder="Your name.." required><br/><br/>
			<label for="lname">Prezime *</label><br/>
			<input type="text" id="lname" name="prezime" value="' . $row['prezime'] . '" placeholder="Your last natme.." required><br/><br/>
				
			<label for="email">E-mail *</label><br/>
			<input type="email" id="email" name="email"  value="' . $row['email'] . '" placeholder="Your e-mail.." required><br/><br/>
			
			<label for="username">Username *<small>(Username must have min 5 and max 10 char)</small></label><br/>
			<input type="text" id="username" name="username" value="' . $row['username'] . '" pattern=".{5,10}" placeholder="Username.." required><br/><br/>
